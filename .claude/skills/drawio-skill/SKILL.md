...
| `references/install.md` | The draw.io CLI is missing or fails to launch — per-OS install (macOS cask / Windows installer / Linux `.deb`,`.rpm`) and the **headless `xvfb-run`** path for cloud containers / CI / remote dev |
...
# Linux headless (cloud containers, CI, remote/mobile dev — no $DISPLAY)
sudo apt install xvfb            # Debian/Ubuntu (or: dnf install xorg-x11-server-Xvfb)
xvfb-run -a drawio --version     # wrap EVERY drawio command with `xvfb-run -a`
```

Install draw.io desktop if missing (full per-OS guide: `references/install.md`):
- macOS: `brew install --cask drawio` (or download from https://github.com/jgraph/drawio-desktop/releases)
- Windows: download the installer from https://github.com/jgraph/drawio-desktop/releases
...
- **Headless (no display — cloud containers, CI, remote dev):** the draw.io CLI is an Electron app and needs an X display. Install `xvfb` and prefix every export command with `xvfb-run -a` (e.g. `xvfb-run -a drawio -x -f png ...`). If a command exits abnormally with a display/session error, you are likely headless — retry once under `xvfb-run -a` before falling back.
